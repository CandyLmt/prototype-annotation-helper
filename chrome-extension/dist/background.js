...
// background.js
Object.assign(globalThis, { fetch: import_cross_fetch.default });
var SUPABASE_URL = "https://bmatatqpfllinpqqvwej.supabase.co";
var SUPABASE_SERVICE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJtYXRhdHFwZmxsaW5wcXF2d2VqIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3OTM3NDE5MCwiZXhwIjoyMDk0OTUwMTkwfQ.q2lAKP3eP06jQGiRCjtMgVdAKu-UIPyBqZwosXliTvY";
...
console.log("\u{1F680} \u539F\u578B\u6807\u6CE8\u52A9\u624B\u540E\u53F0\u670D\u52A1\u542F\u52A8");
console.log("\u{1F517} Supabase URL:", SUPABASE_URL);
async function testSupabaseConnection() {
  console.log("\u{1F504} \u6D4B\u8BD5 Supabase \u8FDE\u63A5...");
  try {
    const { data, error } = await supabase.from("annotations1").select("id").limit(1);
    if (error) {
...
chrome.runtime.onInstalled.addListener(async function() {
  console.log("\u{1F389} \u6269\u5C55\u5B89\u88C5\u6210\u529F");
  chrome.storage.local.set({ annotations: {} });
  await testSupabaseConnection();
});
chrome.runtime.onStartup.addListener(async function() {
  console.log("\u{1F503} \u6D4F\u89C8\u5668\u542F\u52A8\uFF0C\u521D\u59CB\u5316\u6269\u5C55");
  await testSupabaseConnection();
});
chrome.runtime.onMessage.addListener(async function(request, sender, sendResponse) {
  console.log("\u{1F4E8} \u6536\u5230\u6D88\u606F:", request.action);
  try {
    if (request.action === "getUserInfo") {
      chrome.storage.local.get(["userId", "userEmail"], function(stored) {
        ...
        sendResponse({ userId, userEmail });
      });
      return true;
    } else if (request.action === "loadAnnotations") {
      const pageUrl = request.data.pageUrl;
      const userId = request.data.userId;
      const { data, error } = await supabase.from("annotations1").select("*").eq("url_path", pageUrl);
      if (error)
        throw error;
      const annotations = {};
      const idMap = {};
      ...
      let result;
      if (id) {
        const { data: existing } = await supabase.from("annotations1").select("user_id").eq("id", id).single();
        if (existing && existing.user_id && existing.user_id !== userId) {
          sendResponse({ success: false, error: "\u65E0\u6743\u9650\u4FEE\u6539\u6B64\u6807\u6CE8" });
          return;
        }
        const { data, error } = await supabase.from("annotations1").update({ content: text }).eq("id", id).select();
        if (error)
          throw error;
        result = data;
      } else {
        const displayName = userEmail ? userEmail.split("@")[0] : userId || "\u672A\u77E5\u7528\u6237";
        const { data, error } = await supabase.from("annotations1").insert({
          url_path: pageUrl,
      ...
      if (id) {
        ...
        const { error } = await supabase.from("annotations1").delete().eq("id", id);
        if (error)
          throw error;
      }
      ...
      const { error } = await supabase.from("annotations1").delete().eq("url_path", pageUrl).eq("user_id", userId);
      if (error)
        throw error;
      sendResponse({ success: true });
      ...
      let notFoundCount = 0;
      const displayName = userId && userId.includes("@") ? userId.split("@")[0] : userId;
      console.log("\u{1F504} batchDeleteAnnotations - ids:", ids);
      console.log("\u{1F504} batchDeleteAnnotations - userId:", userId);
      console.log("\u{1F504} batchDeleteAnnotations - displayName:", displayName);
      for (const id of ids) {
        const { data: existing, error: fetchError } = await supabase.from("annotations1").select("user_id").eq("id", id).single();
        if (fetchError) {
        ...
        console.log("\u{1F4CB} \u627E\u5230\u6807\u6CE8:", id, "user_id:", existing.user_id);
        if (existing.user_id && existing.user_id !== displayName) {
          console.log("\u274C \u6743\u9650\u4E0D\u8DB3:", id, "owner:", existing.user_id, "current:", displayName);
          failedCount++;
          continue;
        }
        const { error: deleteError } = await supabase.from("annotations1").delete().eq("id", id);
        if (deleteError) {
      ...
      console.log("\u{1F4CA} \u5220\u9664\u7ED3\u679C:", { deletedCount, failedCount, notFoundCount });
      if (deletedCount > 0) {
        sendResponse({ success: true, deletedCount, failedCount, notFoundCount });
      } else {
        let errorMsg = "\u6CA1\u6709\u5220\u9664\u4EFB\u4F55\u6807\u6CE8";
        if (notFoundCount > 0)
          errorMsg += `\uFF08${notFoundCount}\u4E2A\u672A\u627E\u5230\uFF09`;
        if (failedCount > 0)
          errorMsg += `\uFF08${failedCount}\u4E2A\u6743\u9650\u4E0D\u8DB3\uFF09`;
        sendResponse({ success: false, error: errorMsg });
      }
    } else if (request.action === "getAnnotations") {
      const { data, error } = await supabase.from("annotations1").select("*");
      if (error)
        throw error;
      ...
      for (const [selector, annotation] of Object.entries(annotations)) {
        const { data: existing } = await supabase.from("annotations1").select("id").eq("url_path", pageUrl).eq("element_selector", selector).single();
        if (existing) {
          const { error } = await supabase.from("annotations1").update({ content: annotation.text }).eq("id", existing.id);
          if (error)
            throw error;
        } else {
          const { error } = await supabase.from("annotations1").insert({
            url_path: pageUrl,
            element_selector: selector,
